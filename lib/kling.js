...
function isDefaultCreateUrl(baseUrl, createUrl) {
  return createUrl === `${baseUrl}/v1/videos`;
}
...
export async function generateSceneVideo({ scene, references = {}, outputDir }) {
  ...
  try {
    ...
    if (message.includes('failed (404)') && isDefaultCreateUrl(baseUrl, createUrl)) {
      throw new Error(
        `${message} Default endpoint /v1/videos is likely not valid for your OmniVideo account/model. Set KLING_CREATE_URL and KLING_STATUS_URL_TEMPLATE exactly as shown in your Kling API docs.`
      );
    }
    throw error;
  }
