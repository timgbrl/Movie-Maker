...
function getCreateUrl(baseUrl) {
  return (process.env.KLING_CREATE_URL || `${baseUrl}/v1/videos/omni-video`).trim();
}
...
function isDefaultCreateUrl(baseUrl, createUrl) {
  return createUrl === `${baseUrl}/v1/videos/omni-video`;
}

function getOmniVideoPayload(scene) {
  const durationSeconds = Math.max(5, Math.min(10, Number(scene.durationSeconds || 7)));
  return {
    model_name: process.env.KLING_MODEL || 'kling-video-o1',
    prompt: scene.klingPrompt,
    mode: process.env.KLING_MODE || 'pro',
    aspect_ratio: process.env.KLING_ASPECT_RATIO || '1:1',
    duration: String(durationSeconds)
  };
}

function normalizeKlingEnvelope(json, label, url) {
  if (typeof json?.code !== 'undefined' && Number(json.code) !== 0) {
    const serviceCode = json.code;
    const detail = json.message || json.msg || 'Unknown service error';
    const requestId = json.request_id || json.requestId || json.trace_id || json.traceId;
    throw new Error(
      `${label} service error at ${url}: code=${serviceCode}, message=${detail}${requestId ? `, requestId=${requestId}` : ''}`
    );
  }
...
async function fetchJson(url, options, label) {
  ...
  try {
    ...
    return normalizeKlingEnvelope(parsed, label, url);
  } catch (error) {
...
function pickVideoUrl(json) {
  return (
    ...
    json?.data?.video_url ||
    json?.data?.url ||
    json?.data?.resource?.videoUrl ||
    json?.data?.resource?.video_url ||
    null
  );
}

function pickStatus(json) {
  return json?.status || json?.data?.status || json?.task_status || json?.data?.task_status || null;
}
...
export async function generateSceneVideo({ scene, references = {}, outputDir }) {
  ...
  const payload = getOmniVideoPayload(scene);
  ...
  try {
    ...
    if (message.includes('failed (404)') && isDefaultCreateUrl(baseUrl, createUrl)) {
      throw new Error(
        `${message} Default endpoint /v1/videos/omni-video is likely not valid for your OmniVideo account/model. Set KLING_CREATE_URL and KLING_STATUS_URL_TEMPLATE exactly as shown in your Kling API docs.`
      );
    }
    throw error;
  }
...
async function pollKlingVideo(jobId, authorization, retries = 30) {
  ...
  for (let i = 0; i < retries; i += 1) {
    ...
    if (isSuccessStatus(status) && videoUrl) {
      return videoUrl;
    }
